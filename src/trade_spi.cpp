...
void MyTraderSpi::OnDisconnected(uint64_t session_id, int reason)
{
	...
	//断线，向监控端发送断线日志
	char strlog[1024] = { 0 };
	sprintf(strlog, "%lld disconnected, error_id: %d, error_msg: %s.", session_id, error_info->error_id, error_info->error_msg);
	SendMsg(XTP_LOG_LEVEL_ERROR, "Disconnected trade", strlog, 2);
	...
	do
	{
		...
		if (temp_session_id_ != 0)
		{
			//重新登录成功后，向监控端发送日志
			char strlog2[1024] = { 0 };
			sprintf(strlog2, "%s success to reconnect to trade %s:%d, session id is %lld.", account_name.c_str(), trade_server_ip.c_str(), trade_server_port, temp_session_id_);
			SendMsg(XTP_LOG_LEVEL_INFO, "Login trade", strlog2);
		}
...
void MyTraderSpi::OnOrderEvent(XTPOrderInfo *order_info, XTPRI *error_info, uint64_t session_id)
{
	...
	////收到订单响应后，向监控端发送日志
	//char strlog[1024] = { 0 };
	//sprintf(strlog, "%lld status is %d, cancel_xtp_id is %lld, error_id: %d, error_msg: %s.",order_info->order_xtp_id, order_info->order_cancel_xtp_id, error_info->error_id, error_info->error_msg);
	//SendMsg(XTP_LOG_LEVEL_INFO, "OrderEvent", strlog);
	...
	//{
	//	uint64_t cancel_xid = pUserApi->CancelOrder(order_info->order_xtp_id, session_arrary[order_info->order_client_id]);
	//	if (cancel_xid != 0)
	//	{
	//		//报单成功，向监控端发送成功日志
	//		char strlog2[1024] = { 0 };
	//		sprintf(strlog2, "%lld cancel order %lld, original order is %lld.", session_arrary[order_info->order_client_id], cancel_xid, order_info->order_xtp_id);
	//		SendMsg(XTP_LOG_LEVEL_INFO, "CancelOrder", strlog2);
	//	}
	//	else
	//	{
	//		//报单失败，获取报单失败原因
	...
	//		//报单失败，向监控端发送失败日志
	//		char strlog2[1024] = { 0 };
	//		sprintf(strlog2, "%lld failed to cancel order, original order is %lld, error_id: %d, error_msg: %s.", session_arrary[order_info->order_client_id], order_info->order_xtp_id, error_info->error_id, error_info->error_msg);
	//		SendMsg(XTP_LOG_LEVEL_ERROR, "CancelOrder", strlog2, 3);
	//	}
	//	break;
	//}
	...
	//	uint64_t xtp_id = pUserApi->InsertOrder(&(orderList[i]), session_arrary[i]);
	//	if (xtp_id != 0)
	//	{
	//		//报单成功，向监控端发送成功日志
	//		char strlog2[1024] = { 0 };
	//		sprintf(strlog2, "%lld insert order %lld.", session_arrary[i], xtp_id);
	//		SendMsg(XTP_LOG_LEVEL_INFO, "InsertOrder", strlog2);
	//	}
	//	else
	//	{
	//		//报单失败，获取报单失败原因
	...
	//		//报单失败，向监控端发送失败日志
	//		char strlog2[1024] = { 0 };
	//		sprintf(strlog2, "%lld failed to insert order, error_id: %d, error_msg: %s.", session_arrary[i], error_info->error_id, error_info->error_msg);
	//		SendMsg(XTP_LOG_LEVEL_ERROR, "InsertOrder", strlog2, 3);
	//	}
...
void MyTraderSpi::OnTradeEvent(XTPTradeReport *trade_info, uint64_t session_id)
{
	...
	//记录该成功交易的记录uid
	{
		...
		has_trade_ticker[trade_info->ticker] = true;//记录已经成功跟过订单
	}
	myStrategy->CancelTicker(trade_info->ticker, trade_info->order_xtp_id);//多余订单撤单并删除跟单记录
	//收到成交回报后，向监控端发送日志
	char strlog[1024] = { 0 };
	sprintf(strlog, "time:%lld  ticker:%s  %lld trade event.", trade_info->trade_time, trade_info->ticker, trade_info->order_xtp_id);
	SendMsg(XTP_LOG_LEVEL_INFO, "TradeEvent", strlog);
}
...
// 	cout << "cancel_order_xtp_id:" << cancel_info->order_cancel_xtp_id << "   order_xtp_id:" << cancel_info->order_xtp_id << "  errorcode:" << error_info->error_id << "  msg:" << error_info->error_msg << endl;
	...
	//撤单失败，向监控端发送失败日志
	char strlog[1024] = { 0 };
	sprintf(strlog, "order %lld failed to cancel order %lld, error_id: %d, error_msg: %s.", cancel_info->order_cancel_xtp_id, cancel_info->order_xtp_id, error_info->error_id, error_info->error_msg);
	SendMsg(XTP_LOG_LEVEL_ERROR, "CancelOrderError", strlog, 4);
}
...
void MyTraderSpi::OnQueryPosition(XTPQueryStkPositionRsp * investor_position, XTPRI * error_info, int request_id, bool is_last, uint64_t session_id)
{
	...
	//***************卖逻辑*************//
	printf("possessTicker: %s \t%ld\n", investor_position->ticker, investor_position->sellable_qty);
	char msg[128];
	sprintf(msg, "\tTicker: %s \tQty:%ld", investor_position->ticker, investor_position->sellable_qty);
	SendMsg(XTP_LOG_LEVEL_INFO, "possessTicker", msg);

	myStrategy->AddPossessTicker(investor_position->ticker, investor_position->sellable_qty);//将持股信息保存
}
